...
| **Redis memory fills up, causing BullMQ jobs to reject.** | **Redis Out of Memory** | Caches storing session data without eviction policy. | 1. Set Redis config: `maxmemory-policy allkeys-lru`.<br>2. Evict older key metrics; ensure Redis backup logs are persistently written to disk. |
