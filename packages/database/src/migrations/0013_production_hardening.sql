...
ON ai_support_agent.conversations(tenant_id, assignee_id, status);
...
ON ai_support_agent.messages USING gin(to_tsvector('english', coalesce(body, '')));

CREATE INDEX IF NOT EXISTS idx_kb_doc_title_content_gin 
ON ai_support_agent.knowledge_documents USING gin(to_tsvector('english', coalesce(title, '') || ' ' || coalesce(content, '')));
...
ON ai_support_agent.analytics_events(tenant_id, event_type, created_at);
