...
export class AiPlatformClient extends BaseClient {
  ...
  async classifyIntent(tenantId: string, content: string): Promise<AiResponseContract> {
  ...
  async generateResponse(tenantId: string, conversationHistory: any[]): Promise<string> {
  ...
  async ingestDocument(tenantId: string, url: string, type: string): Promise<{ jobId: string }> {
