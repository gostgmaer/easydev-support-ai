...
export class CostTrackerService {
  ...
  async trackAiUsage(tenantId: string, model: string, inputTokens: number, outputTokens: number): Promise<number> {
    ...
    await this.incrementCountMetric(tenantId, 'ai_tokens_count', inputTokens + outputTokens);
  ...
  async trackConnectorUsage(tenantId: string, connectorType: string, bytesTransferred: number): Promise<number> {
    ...
    await this.incrementCountMetric(tenantId, 'connector_bytes_count', bytesTransferred);
  ...
  async trackStorageUsage(tenantId: string, additionalBytes: number): Promise<number> {
    ...
    await this.incrementCountMetric(tenantId, 'storage_bytes_count', additionalBytes);

    await this.checkQuotas(tenantId, 'storage');
    return totalCost;
  }

  private async incrementCostMetric(tenantId: string, metricKey: string, amount: number): Promise<void> {
  ...
  private async incrementCountMetric(tenantId: string, metricKey: string, count: number): Promise<void> {
    if (this.isRedisConnected && this.redisClient) {
      const today = new Date().toISOString().substring(0, 10);
      const key = `cost:${tenantId}:${metricKey}:${today}`;
  ...
  private async checkQuotas(tenantId: string, category: string): Promise<void> {
    ...
    const [ai, conn, stor] = await this.redisClient.mget(aiKey, connectorKey, storageKey);
    const totalDailyCost = (parseFloat(ai || '0') + parseFloat(conn || '0') + parseFloat(stor || '0'));

    // Check against tenant cost limits (default $50/day limit)
    const costLimit = 50.00;
    if (totalDailyCost > costLimit) {
      this.logger.warn(`Tenant ${tenantId} has breached the daily cost budget: $${totalDailyCost.toFixed(2)} / $${costLimit}`);
  ...
  async getUsageMetrics(tenantId: string): Promise<any> {
    if (!this.isRedisConnected || !this.redisClient) return { message: 'Metrics currently unavailable' };

    const today = new Date().toISOString().substring(0, 10);
    const aiCost = parseFloat(await this.redisClient.get(`cost:${tenantId}:ai_cost:${today}`) || '0');
    const connCost = parseFloat(await this.redisClient.get(`cost:${tenantId}:connector_cost:${today}`) || '0');
    const storCost = parseFloat(await this.redisClient.get(`cost:${tenantId}:storage_cost:${today}`) || '0');
    
    const tokens = parseInt(await this.redisClient.get(`cost:${tenantId}:ai_tokens_count:${today}`) || '0', 10);
    const bytes = parseInt(await this.redisClient.get(`cost:${tenantId}:connector_bytes_count:${today}`) || '0', 10);
