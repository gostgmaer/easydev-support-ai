  ...
  async handleJob(job: Job<any, any, string>): Promise<any> {
    ...
    switch (job.name) {
      case 'customer-import-job':
        ...
        return this.customerService.import(tenantId, job.data.records, job.data.userId);

      case 'customer-export-job':
        this.logger.log(`Running customer export job ${job.id}`);
        const csv = await this.customerService.export(tenantId, job.data.format || 'CSV');
        return { length: csv.length };

      case 'customer-metrics-job':
        this.logger.log(`Running customer metrics recalculation for ${job.data.customerId}`);
        return this.metricsService.recalculateMetrics(tenantId, job.data.customerId);

      case 'customer-segmentation-job':
        this.logger.log(`Running customer segmentation check for customer ${job.data.customerId}`);
        const activeSegments = await this.segmentService.findAllSegments(tenantId);
        for (const segment of activeSegments) {
          if (segment.segmentType === 'DYNAMIC') {
            await this.segmentService.runDynamicSegmentation(tenantId, segment.id);
          }
        }
